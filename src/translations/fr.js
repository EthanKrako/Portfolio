export default {
    "lang-abv": "FR",
    "hero-title": "Hey ! Je suis <strong>Ethan</strong>,",
    "hero-subtitle": "Un développeur Full Stack principalement expérimenté en développement web, mais aussi très intéressé par l'architecture des systèmes informatiques et la programmation bas niveau.",
    "hero-subtitle-2": "J'aime comprendre comment les choses fonctionnent à tous les niveaux, et appliquer ces connaissances pour un code maintenable, efficace et sûr.",
    "profile picture": "Photo de profil",
    "project-title": "Projets",
    "project-pro": "Pro",
    "project-perso": "Perso",
    "website-link": "Voir le site ↪",
    "github-link": "Lien GitHub ↪",
    "experience-title": "Expérience",
    "my-work": "Mon travail",
    "about-me": "À propos de moi",
    "languages": "Langues",
    "hobbies": "Centres d'intérêt",

    // Accessibility
    // titles
    "send mail": "Envoyer un e-mail",
    "access github": "Mon profil GitHub",
    "linkedin profile": "Mon profil LinkedIn",
    "see website": "Voir le site",
    "see github": "Voir sur GitHub",
    // alt
    "project screenshot": "Capture d'écran du projet",
    "fullsize project screenshot": "Capture du projet en plein écran",
    "french flag": "Drapeau français",
    "english flag": "Drapeau anglais",
    "japanese flag": "Drapeau japonais",


    // Text blocks
    // Projets
    "pro-title-1": "Odyssea",
    "pro-desc-1": "Création d'un nouveau site web pour Odyssea, en se concentrant principalement sur l'intégration frontend. J'ai travaillé principalement sur la page d'accueil, les landing pages et les actualités. Les principaux challenges de ce projet reposaient sur l'optimisation des performances du site, l'assurance d'une parfaite réactivité sur tous les appareils, et l'intégration appropriée du système de cookies, en particulier pour le contenu vidéo.",

    "pro-title-2": "Seine-Saint-Denis Habitat",
    "pro-desc-2": "Développement d’un script permettant l’import de résidences, ainsi que la création d’une carte interactive et d’une page de listing dédiée. Le projet a impliqué la mise en place d’un système de géocodage, l’utilisation des API de la ville pour afficher les services sur la carte, ainsi que l’implémentation de filtres pour la carte et le listing des résidences.",

    "pro-title-3": "Sporsora",
    "pro-desc-3": "Le but de ce projet était de migrer le site existant de Sporsora vers WordPress. J'ai travaillé à la fois sur le frontend et le backend, pour recréer une copie conforme du site précédent tout en améliorant les performances et la maintenabilité.",

    "perso-title-1": "Tracker de CV envoyés",
    "perso-desc-1": "Une application React/Express de suivi de candidatures d'emploi, pour organiser et suivre les CV envoyés à différentes entreprises. L'objectif premier de ce projet était de pouvoir jouer peu avec les tokens JWT et d'explorer plus en profondeur le fonctionnement interne de React, notamment le fonctionnement des phases de rendu et de commit.",

    "perso-title-2": "Éditeur de texte collaboratif",
    "perso-desc-2": "Un éditeur de texte collaboratif en temps réel construit avec Angular et NestJS. Le but de ce projet était simplement de jouer avec les WebSockets et la synchronisation en temps réel, en utilisant RxJS, Socket.IO et Yjs pour permettre une collaboration fluide, tout en étant capable de sauvegarder et charger les documents depuis la base de données.",

    "perso-title-3": "Serveur HTTP (Projet en cours)",
    "perso-desc-3": "Un serveur HTTP minimaliste construit en C, pour explorer les fondements du protocole HTTP et de la communication réseau.",

    "ws-cdd-title": "CDD - Web Strategies",
    "ws-cdd-desc": "Suite au stage, nous avons décidé de continuer à collaborer pour 4 mois supplémentaires. Grâce à l'expérience acquise, j'ai eu l'occasion de travailler sur des tâches plus complexes telles que la résolution directe de tickets clients ou la mise en production de nouveaux projets.",
    "ws-cdd-duration": "Septembre 2025 - Décembre 2025",

    "master-graduation-title": "Obtention du master Informatique",
    "master-graduation-desc": "Le master m'a permis d'explorer des concepts plus avancés dans la programmation. C'est en suivant des cours autour des patrons de conception, de l'architecture logicielle et des techniques de programmation (premières classes, pureté, etc.) que je me suis découvert une passion pour l'optimisation et la propreté du code sous toutes ses formes.",
    "master-graduation-date": "Septembre 2025",

    "ws-internship-title": "Stage - Web Strategies",
    "ws-internship-desc": "Pour mon stage de fin d'études, j'ai eu l'occasion d'évoluer en tant que développeur full stack sur une grande variété de projets au sein de l'agence. Cette expérience m'a permis de maîtriser les défis liés à la performance, la sécurité et l'accessibilité, tout en m'intégrant pleinement à l'équipe de développeurs.",
    "ws-internship-duration": "Avril 2025 - Aout 2025",

    "isp-internship-title": "Stage - ISP Aquitaine",
    "isp-internship-desc": "Ma première expérience professionnelle en tant que développeur. J'ai travaillé sur un ERP interne dans un environnement WAMP. L'essentiel du travail consistait à comprendre les besoins de l'entreprise et manipuler, formater, afficher et stocker les données de l'ERP.",
    "isp-internship-duration": "Avril 2022 - Juin 2022",


    // About me
    "about-me-desc-1": "Je suis un développeur habitant proche de Bordeaux, en France.",
    "about-me-desc-2": "J'ai toujours aimé l'apprentissage sous toutes ses formes, comme en témoigne mon attrait pour l'informatique, mais également mes nombreux autres passe-temps comme les langues étrangères, le sport et les jeux vidéo.",
    "about-me-desc-3": "J'aime rencontrer du monde, apprendre aux autres et des autres, et j'essaie le plus possible de partager ma bonne humeur et ma motivation avec les gens qui m'entourent. J'aime être challenger et profiter de ces situations pour apprendre et m'améliorer.",

    // Language levels
    "french": "Français",
    "french-level": "Natif",

    "english": "Anglais",
    "english-level": "Courant",
    "english-description": "C1-C2",

    "japanese": "Japonais",
    "japanese-level": "Conversation basique",
    "japanese-description": "N4",

    // Hobbies
    "tricking": "Tricking",
    "motorcycles": "Moto",
    "music": "Musique",
    "languages": "Langues étrangères",
    "chess": "Échecs",
    "video-games": "Jeux vidéo",
    "hiking-running": "Randonnée / Course à pied",
    "climbing": "Escalade",
}